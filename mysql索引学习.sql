...
始终记住三个原则:
       ...
       2、按顺序访问范围数据是很快的。第一 顺序IO不需要多次磁盘寻道，所以比随机IO要快很多(特别是对机械硬盘)	。第二 如果服务器能够按需要熟顺序读取数据，
        那么就不需要额外的排序操作，并且group by 查询也无须在做排序和将行按组进行聚合计算了。
       3、索引覆盖查询是很快的。如果一个索引包含了查询需要的所有列，那么存储引擎就不需要再回表查找行。这避免了大量的单行访问。		
理解索引时如何工作的非常重要，应该根据这些理解来创建最合适的索引，
而不是根据一些诸如"在所列索引中将选择性最高的列放在第一列"或"应该为where子句中出现的所有列创建索引"之类的经验
法则及其推论
